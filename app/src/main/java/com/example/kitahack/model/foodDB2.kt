...
class foodDB2(appContext: Context?) {
    ...
    fun getListObject(key: String?): ArrayList<PopularFoodAdapterClass2> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList = ArrayList<PopularFoodAdapterClass2>()
        for (jObjString in objStrings) {
            val player = gson.fromJson(jObjString, PopularFoodAdapterClass2::class.java)
    ...
    fun putListObject(key: String?, playerList: ArrayList<PopularFoodAdapterClass2>) {
        checkForNullKey(key)
        val gson = Gson()
