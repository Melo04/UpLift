...
class foodDB3(appContext: Context?) {
    ...
    fun getListObject(key: String?): ArrayList<RestaurantModel> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList = ArrayList<RestaurantModel>()
        for (jObjString in objStrings) {
            val player = gson.fromJson(jObjString, RestaurantModel::class.java)
    ...
    fun putListObject(key: String?, playerList: ArrayList<RestaurantModel>) {
        checkForNullKey(key)
        val gson = Gson()
