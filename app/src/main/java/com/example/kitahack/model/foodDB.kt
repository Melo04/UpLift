...
class foodDB(appContext: Context?) {
    ...
    fun getListObject(key: String?): ArrayList<PopularFoodAdapterClass> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList = ArrayList<PopularFoodAdapterClass>()
        for (jObjString in objStrings) {
            val player = gson.fromJson(jObjString, PopularFoodAdapterClass::class.java)
    ...
    fun putListObject(key: String?, playerList: ArrayList<PopularFoodAdapterClass>) {
        checkForNullKey(key)
        val gson = Gson()
